...
export function removeOldStops(stopMatches: MatchedBusStop[], allCandidateNodes: readonly Node[], osmChange: OsmChangeFile) {
  const stopIds = new Set(stopMatches.flatMap(({ stop }) => [ stop.stop_id, stop.stop_code ]))

  const nodesToDelete = allCandidateNodes
    .filter(node => (
      (
        node.tags["operator"]?.includes("King County Metro") ||
        node.tags["network"]?.includes("King County Metro")
      )
        &&
      (
        (
